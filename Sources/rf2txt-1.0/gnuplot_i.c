...
void gnuplot_setfilledstylefilled(gnuplot_ctrl * h, char * color, char * above)
{
		sprintf(h->pstyle,"filledcurves %s x1 lt rgb '%s' lw 2",above,color);
    //strcpy(h->pstyle, plot_style) ;

    return ;
}

void gnuplot_setLinesstylefilled(gnuplot_ctrl * h, char * color)
{
	sprintf(h->pstyle,"lines lt rgb '%s' lw 2",color);
    //strcpy(h->pstyle, plot_style) ;
...
void gnuplot_set_format_x(gnuplot_ctrl * h,char * label)
{
	char cmd[GP_CMD_SIZE] ;
	strcpy(cmd,"set format x \"");
	strcat(cmd, label);
	strcat(cmd, "\"\n");
	fputs(cmd, h->gnucmd) ;
    fflush(h->gnucmd) ;
...
void gnuplot_set_timefmt(gnuplot_ctrl * h,char * label)
{
	char cmd[GP_CMD_SIZE] ;
	strcpy(cmd,"set timefmt \"");
	strcat(cmd, label);
	strcat(cmd, "\"\n");
	fputs(cmd, h->gnucmd) ;
    fflush(h->gnucmd) ;
...
)
{
    ...
    for (i=0 ; i<n ; i++) {
		...
		{
		     fprintf(stderr,
		                "write error in temporary files") ;
		        return ;
		}
    }
    close(tmpfd) ;
...
)
{
    ...
    for (i=0 ; i<n; i++) {
    	struct tm *tb;
    	tb = localtime(&x[i]);
    	sprintf(line, "%d-%02d-%02d:%02d:%02d:%02d %g\n",tb->tm_year+1900, tb->tm_mon+1,tb->tm_mday,tb->tm_hour,tb->tm_min,tb->tm_sec, y[i]) ;
		//printf("%s\n",line);
    	if(write(tmpfd, line, strlen(line))<0)
    	{
    	     fprintf(stderr,
    ...
    //printf("%s\n",name);
    if (title == NULL) {
        sprintf(line, "%s \"%s\" using 1:2 with %s", cmd, name, handle->pstyle) ;
    } else {
        sprintf(line, "%s \"%s\" using 1:2 title \"%s\" with %s", cmd, name,
                      title, handle->pstyle) ;
    }
   // printf("%s\n",line);
    /* send command to gnuplot  */
	gnuplot_cmd(handle, "set xdata time");
	gnuplot_set_timefmt(handle, "%Y-%m-%d:%H:%M:%S");
